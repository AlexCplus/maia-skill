...
import { SECTOR_COLORS, SECTOR_SHORT_NAMES } from "@/lib/constants"

interface PickCardProps {
  pick: RiskAdjustedPick
...
export function PickCard({ pick, sectors }: PickCardProps) {
  ...
  const confLabel = pick.risk_adjusted_score ? "Risk-Adj Score" : "Confidence"
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-xl border border-[#E6E6E4] bg-[#FCFCFB] p-5 transition-all hover:border-[#D0D0CE] hover:shadow-md"
    >
      <div className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-[#37352F] text-xs font-bold text-white">
        #{pick.rank}
      </div>
      ...
      <span
        ...
        style={{
          backgroundColor: `${SECTOR_COLORS[pick.sector]}15`,
          color: SECTOR_COLORS[pick.sector],
        }}
      >
        {SECTOR_SHORT_NAMES[pick.sector] ?? pick.sector}
      </span>
      ...
      {price && (
        <div className="mb-2 text-2xl font-bold text-[#252420]">{price}</div>
      )}

      <div className="mb-3 flex flex-wrap gap-1.5">
        <span
          className={`inline-block rounded-full border px-3 py-0.5 text-xs font-semibold uppercase ${recStyles[pick.recommendation]}`}
        >
          {pick.recommendation}
        </span>
        {pick.risk_score != null && (
          <span className="inline-block rounded-full border border-[#E6E6E4] bg-[#F7F7F5] px-3 py-0.5 text-xs text-[#8B8B85]">
            Risk {pick.risk_score}/10
          </span>
        )}
        {pick.position_size && (
          <span className="inline-block rounded-full border border-blue-200 bg-blue-50 px-3 py-0.5 text-xs text-blue-700">
            {pick.position_size}
          </span>
        )}
      </div>
