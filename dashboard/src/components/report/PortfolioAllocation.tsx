...
export function PortfolioAllocation({ allocation }: { allocation: AllocationType }) {
  ...
  return (
    ...
    >
      <h3 className="mb-4 text-base font-semibold text-[#252420]">
        Recommended Portfolio Allocation
      </h3>
      <div className="space-y-3">
        {entries.map(([key, value], i) => (
          <div key={key} className="flex items-center gap-3">
            <span className="w-20 text-sm font-medium capitalize text-[#4D4A44]">{key}</span>
            <div className="relative h-6 flex-1 overflow-hidden rounded-full bg-[#F0F0ED]">
              <motion.div
                ...
                className="flex h-full min-w-[40px] items-center justify-end rounded-full pr-2.5 text-xs font-bold text-white"
                style={{ backgroundColor: SECTOR_COLORS[key] ?? "#8B8B85" }}
              >
                {value}%
              </motion.div>
            </div>
